...
async function runTest() {
  ...
  if (!content.includes("<rss version=\"2.0\"")) {
    throw new Error("rss.xml does not contain <rss version=\"2.0\"");
  }

  if (!content.includes("<title>Litapp Blog</title>")) {
    throw new Error("rss.xml does not contain correct title");
  }

  if (!content.includes("<item>")) {
    throw new Error("rss.xml does not contain any items");
  }
